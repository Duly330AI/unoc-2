...
const buildQueuedEnvelope = (kind: string, payload: unknown, includeTopoVersion = true, correlationId?: string): QueuedEnvelope => {
  ...
  const envelope: QueuedEnvelope = {
    ...
    ...(includeTopoVersion ? { topoVersion: topologyVersion } : {}),
  };
...
const emitQueuedEnvelope = (envelope: QueuedEnvelope) => {
  io.emit("event", serializeEnvelope(envelope));
};
...
export const flushRealtimeOutbox = (bucketId?: string) => {
  ...
  for (const currentBucketId of bucketIds) {
    ...
    for (const phase of [1, 2, 3, 4, 5] as const) {
      ...
      for (const entry of appendEntries.sort((a, b) => a.seq - b.seq)) {
        emitQueuedEnvelope(entry.envelope);
      }
          ...
          .map((entry) => ({
            ...
            envelope: {
              ...
              payload: {
                ...
                items: Array.from(entry.itemsByKey.values()).sort((a, b) =>
...
const clearRealtimeOutbox = (bucketId?: string) => {
  if (bucketId) {
...
export const emitEvent = (kind: string, payload: unknown, includeTopoVersion = true, correlationId?: string) => {
